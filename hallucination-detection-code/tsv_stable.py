...
def train_model(model, optimizer, device, prompts, labels, args):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_start in tqdm(range(0, num_samples, batch_size), desc=f"Epoch {epoch+1}/{num_epochs} Batches", leave=False):
            ...
            with autocast(dtype=torch.float16):
                ...
                hidden_states = hidden_states  # MEMFIX: keep tuple, index layer directly (was torch.stack all layers -> OOM)
        ...
        if (epoch+1) % 1 == 0:
            ...
            if test_auroc > best_test_auroc:
                ...
                import numpy as _np; _np.save("/root/tsv/test_scores.npy", _np.array(test_predictions)); _np.save("/root/tsv/test_labels.npy", _np.array(test_labels_combined))
    ...
    import os as _os
    anchor_centroids = centroids.detach().clone()
    ...
    with autocast(dtype=torch.float16):                   
        for epoch in range(num_epochs):
            ...
            for batch_start in tqdm(range(0, num_samples, batch_size), desc=f"Epoch {epoch+1}/{num_epochs} Batches", leave=False):
                ...
                with torch.no_grad():
                   ...
                   if _os.environ.get("TSV_STABLE")=="1":
                       _sim=torch.nn.functional.cosine_similarity(anchor_centroids[1:2].float(), centroids.float(), dim=1)
                       if _sim[0] < _sim[1]:
        ...
        try:
            ...
            _fp=_np2.array(test_predictions); _fl=_np2.array(test_labels_combined)
            _np2.save("/root/tsv/test_scores_final.npy", _fp); _np2.save("/root/tsv/test_labels_final.npy", _fl)
            from sklearn.metrics import roc_auc_score as _AUC
            _raw=_AUC(_fl,_fp); print("FINAL-EPOCH AUROC:", round(float(_raw),4), "| flipped:", round(float(max(_raw,1-_raw)),4))
            _exs,_exl = test_model(model, centroids, exemplar_prompts_, exemplar_labels_, device, batch_size, layer_number)
            _exs=_np2.array(_exs); _exl=_np2.array(_exl); _exa=_AUC(_exl,_exs); _flip = _exa < 0.5
            _orient = (1.0-_fp) if _flip else _fp; _oa=_AUC(_fl,_orient)
            _np2.save("/root/tsv/test_scores_oriented.npy", _orient)
            print("EXEMPLAR-ORIENTED AUROC:", round(float(_oa),4), "(exemplarAUROC=", round(float(_exa),4), "flip=", bool(_flip), ")")
        except Exception as _e:
            print("final-save err", _e)
...
HF_NAMES = {
    'llama3.1-8B': 'NousResearch/Meta-Llama-3.1-8B',
...
def main(): 
    ...
    if args.gene:
        ...
        if args.dataset_name in ('tqa','triviaqa','nq_open'):
            args.thres_gt = 0.5
        ...
        optimizer = torch.optim.AdamW(list(tsv.parameters()), lr=args.lr, weight_decay=args.weight_decay)
