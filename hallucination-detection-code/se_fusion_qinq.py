...
tsv=np.load('/root/tsv/test_scores_qi_nq.npy').astype(float); lab=np.load('/root/tsv/test_labels_qi_nq.npy').astype(int)
...
for qi in test_idx:
    f=f'{ans}/batch_generations_hal_det_qwen2.5-7B-instruct_nq_open_answers_index_{qi}.npy'
...
print(out); open('/root/tsv/SE_FUSION_QINQ_RESULT.txt','a').write('\nQwenInstruct-nq_open NLI-SE: '+out+'\n')
