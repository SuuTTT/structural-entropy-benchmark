...
idx=np.load('/root/tsv/data_indices/data_index_tqa.npy'); wild=set(idx[:int(0.75*817)].tolist())
test_idx=[i for i in range(817) if i not in wild]
ans='/root/tsv/save_for_eval/tqa_hal_det/answers'; se=[]
for qi in test_idx:
...
print(out); open('/root/tsv/SE_FUSION_RESULT.txt','a').write('\nNLI-based SE: '+out+'\n')
