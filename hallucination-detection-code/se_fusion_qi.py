...
tsv=np.load('/root/tsv/test_scores_qweninstruct_triviaqa.npy').astype(float); lab=np.load('/root/tsv/test_labels_qweninstruct_triviaqa.npy').astype(int)
idx=np.load('/root/tsv/data_indices/data_index_triviaqa.npy'); wild=set(idx[:300].tolist())
test_idx=[i for i in range(400) if i not in wild]
print('len(test_idx)',len(test_idx),'len(tsv)',len(tsv),'len(lab)',len(lab))
ans='/root/tsv/save_for_eval/triviaqa_hal_det/answers'; se=[]
for qi in test_idx:
    f=f'{ans}/batch_generations_hal_det_qwen2.5-7B-instruct_triviaqa_answers_index_{qi}.npy'
...
print(out); open('/root/tsv/SE_FUSION_QI_RESULT.txt','a').write('\nQwenInstruct-TriviaQA NLI-SE: '+out+'\n')
